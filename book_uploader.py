...
# @Last Modified time: 2025-06-18 22:16:40
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
书单上传工具
...
class BookUploader:
    ...
    def setup_ui(self):
        ...
        # 设置列标题和宽度
        column_widths = {"文件名": 200, "大小": 100, "修改时间": 150, "书籍数量": 80}
        for col in columns:
    ...
    def scan_json_files(self):
        ...
        if not json_files:
            # 如果没有找到JSON文件，显示提示
            self.tree.insert("", "end", values=("未找到JSON文件", "", "", ""))
            return
        ...
        for file_path in json_files:
            try:
                ...
                # 读取JSON文件获取书籍数量
                book_count = self.get_book_count(file_path)
    ...
    def get_book_count(self, file_path):
        """获取JSON文件中的书籍数量"""
        try:
    ...
    def on_file_double_click(self, event):
        ...
        if selection:
            ...
            # 检查是否是有效文件
            if file_name == "未找到JSON文件" or file_name == "读取失败":
                return
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if result:
                    ...
                    result_text += f"📊 书籍数量: {self.get_book_count(file_path)} 本\n"
