 ...
 * @Last Modified time: 2025-06-26 11:04:00
 */
// ==UserScript==
// @name         v2ob哔哩哔哩自动解析下载
        ...
        function attempt() {
            try {
                ...
                if (videoUrl && videoUrl.includes('.mp4')) {
                    // 用window.open方式下载并带上文件名参数
                    const urlWithName = videoUrl + (videoUrl.includes('?') ? '&' : '?') + 'download=' + encodeURIComponent(filename);
                    window.open(urlWithName, '_blank');
                    // 兼容部分浏览器，仍保留a标签download提示
