 ...
 * @Last Modified time: 2025-06-26 10:55:24
 */
// ==UserScript==
// @name         v2ob哔哩哔哩自动解析下载
...
(function () {
    ...
    const handleClipboard = debounce(async function () {
        try {
            ...
            // 只提取目标平台的URL
            const urlMatch = text.match(/https?:\/\/(b23\.tv|bilibili\.com|douyin|xiaohongshu|ixigua|pipix\.com|acfun|toutiao)[^\s"'<>]*/i);
            console.log('[v2ob] 匹配到的URL:', urlMatch ? urlMatch[0] : null);
            if (urlMatch) {
                lastClipboardText = text;
                let url = urlMatch[0];
                console.log('[v2ob] 最终用于解析的URL:', url);
                await autoParse(url);
            }
        } catch (e) {
            // 已有alert
