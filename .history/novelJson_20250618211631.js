 ...
 * @Last Modified time: 2025-06-18 21:16:27
 */
// ==UserScript==
...
(function() {
    ...
    // 通过代理服务器发送请求（解决HTTPS/HTTP问题）
    async function sendRequestViaProxy(url, formData) {
        // 方案1：尝试使用公共代理服务
        const proxyUrls = [
        ...
        try {
            ...
            try {
                ...
                try {
                    ...
                    console.log('HTTP也失败，尝试代理...');
                    // 方式3：通过代理
                    response = await sendRequestViaProxy('http://static.sy.yesui.me:7747/shuyuan', formData);
                    successMethod = '代理';
                }
            }
            
            console.log('网络响应状态:', response.status);
            console.log('成功方式:', successMethod);
            ...
            if (response.ok) {
                ...
                try {
                    if (result && result.trim() !== '') {
                        downloadUrl = 'http://static.sy.yesui.me:7747/shuyuan/' + result;
                    }
                } catch (e) {
                ...
                if (downloadUrl) {
                    alert(`网络导出成功！(${successMethod})\n下载链接：${downloadUrl}`);
                    window.open(downloadUrl, '_blank');
                } else {
                    alert(`网络导出成功！(${successMethod})，但无法获取下载链接！`);
                }
            } else {
            ...
            // 提供解决方案
            let errorMessage = '网络导出失败！\n\n';
            if (error.message.includes('Mixed Content')) {
                errorMessage += '原因：HTTPS页面无法访问HTTP接口\n';
                errorMessage += '解决方案：\n';
                errorMessage += '1. 使用本地导出\n';
                errorMessage += '2. 联系网站管理员启用HTTPS\n';
                errorMessage += '3. 使用浏览器插件禁用混合内容限制';
            } else if (error.message.includes('Failed to fetch') || error.message.includes('ERR_TIMED_OUT')) {
                errorMessage += '原因：网络连接超时或服务器不可用\n';
                errorMessage += '解决方案：\n';
                errorMessage += '1. 检查网络连接\n';
                errorMessage += '2. 稍后重试\n';
                errorMessage += '3. 使用本地导出';
            } else {
                errorMessage += `原因：${error.message}\n`;
                errorMessage += '建议使用本地导出功能';
            }
    ...
    async function testNetworkInterface() {
        try {
            ...
            if (error.message.includes('Mixed Content')) {
                ...
                errorMessage += '建议：\n';
                errorMessage += '1. 使用本地导出\n';
                errorMessage += '2. 安装浏览器插件禁用混合内容限制\n';
                errorMessage += '3. 联系网站管理员启用HTTPS';
            } else if (error.message.includes('Failed to fetch') || error.message.includes('ERR_TIMED_OUT')) {
                errorMessage += '原因：网络连接超时或服务器不可用\n';
                errorMessage += '建议：\n';
                errorMessage += '1. 检查网络连接\n';
